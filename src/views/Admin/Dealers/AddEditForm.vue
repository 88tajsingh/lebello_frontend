<template>
    <DefaultCard :cardTitle="form.id ? `Edit Dealer` : `Add New Dealer`">
        <DomainComponent @customChange="(id) => (form.domain_id = id)" @domainArray="(array) => form.domain_all = array" :deleteService="DealersServices.deleteDealer"
            masterKey="master_dealer_id" :masterDeleteService="DealersServices.deleteMasterDealer" routeTo="dealer">
        </DomainComponent>
        <template v-if="form.id" v-slot:header>
            <MasterSlugForm :form="form" @update-slug="() => fetchPagesData()"
                :SlugUpdateservices="DealersServices.masterSlugDealerUpdate" masteridKeyName="master_dealer_id" />
        </template>
                ...
                <div class="col-span-8">
                    ...
                    <div class="mt-3">
                        <Accordion :open="true" header="Day time banner Gallery images">
                            <div class="mx-3 flex">
                                <SingleCheck v-if="form.id" label="" v-model="checkedFields.day_banner_images">
                                </SingleCheck>
                    ...
                    <div class="mt-3">
                        <Accordion :open="true" header="Day time banner Gallery images">
                            <div class="mx-3 flex">
                                <SingleCheck v-if="form.id" label="" v-model="checkedFields.night_banner_images">
                                </SingleCheck>
                                <div class="col-span-2 w-full border border-gray rounded-lg">
                                    <div class="mt-2 ml-3">
                                        <div class="text-gray-4 text-[13px]">images</div>
                                        ...
                                        <button @click="() => (imageData.night_banner_images.isOpen = true)"
                                            type="button"
                                            class="flex px-3 py-1 col-span-2 mt-5 mb-4 ml-4 justify-center rounded bg-primary font-medium text-gray hover:bg-opacity-90">
                                            Gallery
                                        </button>
                                    ...
                                    <div class="h-auto">
                                        ...
                                        <div class="flex w-full h-auto">
                                            ...
                                            <div class="py-2 rounded-lg px-2 w-full border border-stroke"
                                                ...
                                                {{ imageData.company_logo.mediaName }}
                                            </div>
                                        </div>
                                    ...
                                    <div class="h-auto">
                                        ...
                                        <div class="flex w-full h-auto">
                                            ...
                                            <div class="py-2 rounded-lg w-full px-2 border border-stroke"
                                                ...
                                                {{ imageData.thumb_image.mediaName }}
                                            </div>
                                        </div>
...
const { handleGlobalUpdate  } = useGlobalUpdate(DealersServices.globalDealerUpdate,'master_dealer_id')
// reactive state
const errors = ref({})
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (status === 200 && data.success) {
            if (hasCheckedFields) {
                handleGlobalUpdate(form,checkedFields,'/dealer')
            } else {
