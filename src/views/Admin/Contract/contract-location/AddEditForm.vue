<template>
    <DefaultCard  :cardTitle="form.id ? `Edit Contract Location` : `Add Contract Location`">
      ...
      :deleteService="ContractServices.deleteContractLocation" masterKey="master_contract_location_id"
      :masterDeleteService="ContractServices.deleteMasterContractLocation" routeTo="contract-location"></DomainComponent>

    <template v-if="form.id" v-slot:header>
      <MasterSlugForm :form="form" @update-slug="fetchContractLocationData"
        :SlugUpdateservices="ContractServices.masterContractLocationSlugUpdate" masteridKeyName="master_contract_location_id" />
    </template>
    <form @submit.prevent="handleSubmit">
      <div class="p-6.5 grid grid-cols-2 gap-6">
        <div class="flex flex-col">
          <TextInput type="text" :class="{ 'border-red': errors.name }" v-model="form.contract_location" :errMessage="errors.contract_location"
            @update:modelValue="clearError('contract_location')" label="Name" :hasCheckBox="checkBoxFlag"
            @update:checkValue="value => checkedFields.contract_location = value" />
        ...
        <div class="flex flex-col">
          ...
          <Select :options="contractLoction" :defaultZero="true" label="Parent Contract Location" showfield="contract_location"
          class="w-full" valueField="id" :errorClass="selectError" @update:modelValue="clearError('parent_contract_location')"
          errMessage="Should not be own parent" v-model="form.parent_contract_location" />
