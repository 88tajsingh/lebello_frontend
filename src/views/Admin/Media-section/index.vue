<template>
    <div class="bg-white h-[80vh] pl-3 pb-3">
        <div class="pt-4">
        ...
        <div class="flex flex-wrap gap-3">
            <div v-for="folder in folders" :key="folder" class="border border-gray text-center w-[80px] px-2">
                ...
                <span class="m-auto break-all text-wrap w-[20px]">{{ folder?.folder_name }}</span>
            ...
            <div v-for="media in mediaData" :key="mediaData.id" 
                class="border relative border-gray text-center w-[80px] px-2">
                <label class="" @click="mediaEdit(media)">
                    <img v-if="isImage(media.file_url)" :src="filePath(media.file_url)" alt="Image"
                        class="w-20 h-20" />
                    <img v-else-if="isPdf(media.file_url)" class="w-20 h-20" src="../../../assets/Icons/pdf.svg" alt="PDF Document" />
                    ...
                    <img v-else-if="isExcel(media.file_url)" class="w-20 h-20" src="../../../assets/Icons/msExcel.svg"
                        alt="Excel Document" />
                    <img v-else-if="isVideo(media.file_url)" class="w-20 h-20" src="../../../assets/Icons/video.svg" alt="Video File" />
                    <span v-else>Unknown file format.</span>
                    <!-- <img v-if="isImage(media.file_url)" :src="media.file_url" alt="Image" class="w-full h-auto mb-2" />
                   <embed v-else-if="isPdf(media.file_url)" :src="media.file_url" type="application/pdf" class="w-full h-[300px] mb-2" />
                   <iframe v-else-if="isWord(media.file_url)" :src="media.file_url" class="w-full h-[300px] mb-2"></iframe>
<iframe v-else-if="isExcel(media.file_url)" :src="media.file_url" class="w-full h-[300px] mb-2"></iframe>
<video v-else-if="isVideo(media.file_url)" controls class="w-full h-[300px] mb-2">
  <source :src="media.file_url" type="video/mp4">
  Your browser does not support the video tag.
</video>
<span v-else>Unknown file format.</span> -->
                    <span class="m-auto break-all text-wrap w-[20px]">{{ media.title }} </span>
                    <div class="m-auto absolute top-0 right-0 " @click.stop="() => {
                        mediaDeleteModal();
        ...
        <div class="flex">
            <div class="w-4/6 px-5 h-[100%]">
                <template v-if="isImage(editMediaData.file_url)">
                    <img :src="editMediaData.file_url" alt="Image" />
                </template>
                <template v-else-if="isPdf(editMediaData.file_url) || isWord(editMediaData.file_url) || isExcel(editMediaData.file_url)">
                    <iframe :src="getGoogleDocsViewerUrl(editMediaData.file_url)" width="100%" height="600px"></iframe>
                </template>
            ...
            <div class="w-2/6 border">
                <TextInput id="editTitle" type="text" class="block w-[180px] mr-2 h-[33px]" v-model="editMediaData.title" placeholder="Title" label="Title" :errMessage="errorMessage" />
                <TextInput id="editCaption" type="text" class="block w-[180px] mr-2 h-[33px]" v-model="editMediaData.caption" placeholder="Caption" label="Caption" :errMessage="errorMessage" />
                <TextInput id="editAltText" type="text" class="block w-[180px] mr-2 h-[33px]" v-model="editMediaData.alternative_text" placeholder="Alternative Text" label="Alternative Text" :errMessage="errorMessage" />
                <TextInput id="editDescription" type="text" :isTextarea="true" rows="4" class="block w-[180px] mr-2 h-[33px]" v-model="editMediaData.description" placeholder="Description" label="Description" :errMessage="errorMessage" />
                <Button class="px-6 m-0" bg_th_color="py-2 text-white bg-[#2271B1] hover:bg-[#0a4b78]" @click="handleEditMedia">
                    Save
                </Button>
...
const newFolder = ref()
const folderLocation = ref(0)
const mediaFIle = ref()
const errorMessage = ref()
const selectedMediaFolders = ref()
