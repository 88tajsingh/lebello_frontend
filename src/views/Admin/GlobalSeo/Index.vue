<template>
    <PageHeader> Global SEO / Meta Tags </PageHeader>
    <div class="flex content-between justify-between px-1 mb-2">
        ...
        <div class="flex">
            ...
            <Button @click="() => { router.push({ name: 'global-meta-tag-form' }); store.dispatch('clearEditData'); }"
                class="px-2 py-2">Add Global Tag</Button>
        </div>
    </div>
    <div class="bg-white rounded-[20px]">
        <vue3-datatable class="next-prev-pagination" ref="datatable" skin="bh-table-striped bh-table-hover "
            :hasCheckbox="true" :cloneHeaderInFooter="true" :stickyHeader="false" :rows="rows" :columns="MetaTagCols"
...
const changePages = (page) => {
    ...
    handleGetGlobalMetaTag(payload);
}
// api calls
const handleGetGlobalMetaTag = async (payload) => {
    getLoading.value = true;
    try {
        const res = await GlobalMetaTags.getMetaTags(payload);
...
const handleDeleteGlobalMetaTag = async () => {
    loading.value = true;
    try {
        const res = await GlobalMetaTags.deleteMetaTags({ id: editData.value });
        if (res.status === 200 && res.data.success) {
...
const handleBulkActions = async () => {
    ...
    if (bulkActionSelected.value === 'Delete') {
        loading.value = true;
        try {
            const res = await GlobalMetaTags.bulkDeleteMetaTags({ id: ids });
            if (res.status === 200 && res.data.success) {
                showToast(res.data.message, 'success');
                await handleGetGlobalMetaTag({ domain_id: domain_id.value });
            }
            else if (res.status === 400) {
                showToast(res.data.message, 'error');
            }
