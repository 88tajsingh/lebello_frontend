<template>
  <DefaultCard :cardTitle="store.getters.editData ? `Edit Pages` : `Add Pages`">
    <DomainComponent @customChange="(id) => form.domain_id = id" @domainArray="(array) => form.domain_all = array" :deleteService="PagesServices.deletePages"
      masterKey="master_page_id" :masterDeleteService="PagesServices.deletePageMaster" routeTo="pages">
    </DomainComponent>
    <template v-if="form.id" v-slot:header>
      <MasterSlugForm :form="form" @update-slug="() => fetchPagesData()"
        :SlugUpdateservices='PagesServices.masterPageSlugUpdate' masteridKeyName='master_page_id' />
    </template>
    <form @submit.prevent="handleSubmit">
      <div class="p-6.5 grid grid-cols-2 gap-2">
        ...
        <div class="flex flex-col">
          <TextInput type="text" class="block mr-2 h-[40px] w-full" placeholder="" v-model="form.slug" disabled="true"
        ...
        <div class="flex flex-col">
          ...
          <TextInput type="text" class="block mr-2 h-[40px] w-full" :class="{ 'border-red-500': errors.seo_title }"
            v-model="form.seo_title" :errMessage="errors.seo_title" :hasCheckBox="checkBoxFlag"
        ...
        <div class="col-span-1 w-full mt-1">
          ...
          <TextInput type="text" class="block mr-2 w-full" v-model="form.template" :hasCheckBox="checkBoxFlag"
            @update:checkValue="(value) => { checkedFields.template = value }" />
        ...
        <div class="col-span-2 w-full">
          ...
          <TextInput type="text" class="block mr-2 w-full" :class="{ 'border-red-500': errors.seo_meta_description }"
            :isTextarea="true" :rows="3" v-model="form.seo_meta_description" :errMessage="errors.seo_meta_description"
        ...
        <div class="col-span-1 w-full">
          ...
          <TextInput type="text" class="block mr-2 w-full" :isTextarea="true" :rows="3" v-model="form.seo_meta_keywords"
            :errMessage="errors.seo_meta_keywords" :hasCheckBox="checkBoxFlag"
        ...
        <div class="col-span-1 w-full">
          ...
          <TextInput type="text" class="block mr-2 w-full" :class="{ 'border-red-500': errors.page_tagline }"
            ...
            :hasCheckBox="checkBoxFlag" @update:checkValue="(value) => { checkedFields.page_tagline = value }" />
          <p class="text-sm text-[#646970] text-[11.5px]">Type the page tagline here.</p>
        </div>
        <div class="col-span-1 w-full">
          <input-label for="parent_page" value="Parent Page" />
          <TextInput type="text" class="block mr-2 w-full" v-model="form.parent_page" :hasCheckBox="checkBoxFlag"
        ...
        <div class="col-span-1 w-full">
          ...
          <Select :options="trueFalse" showfield="name" class="w-full" valueField="value" label="Options"
            v-model="form.comments" :hasCheckBox="checkBoxFlag"
            @update:checkValue="(value) => { checkedFields.comments = value }" />
        </div>

        <div class="col-span-1 w-full">
          <input-label for="page_order" value="Page Order" />
          <TextInput type="text" class="block mr-2 w-full" v-model="form.page_order" :hasCheckBox="checkBoxFlag"
