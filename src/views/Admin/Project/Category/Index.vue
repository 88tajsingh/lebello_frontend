<template>
  ...
  <div class="flex  content-between justify-between   mb-2">
    ...
    <div class="flex rounded-lg bg-transparent">
      ...
      <Button @click="() => {router.push({ name: 'Project-category-add'}) }"class="px-2 py-2 m-auto whitespace-nowrap">Add Project Category</Button>
    </div>  
  </div>
    ...
    :rows="data" :columns="proectCategoryCols" :loading="dataTableLoding" :totalRows="totalRows" :isServerMode="true" :pageSize="10" :search="search" @change="changePage">
      ...
      <template #actions="data">
        <div class="flex gap-3">
          <div @click="() => { router.push({ name: 'Project-category-edit', params: { id: data.value.id , domain: data.value.domain_id} }) }" id="edit svg">
            <EditSvg />
          </div>
...
x
  ...
  <DeleteModal v-model:isOpen="deleteModalIsOpen" :modalTitle="'Delete Material'" @delete="handleDeleteProductSeries">
