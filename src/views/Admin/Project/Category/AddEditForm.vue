...
const form = ref({ parent_project_category:0})
const errors = ref({})
...
const handleSubmit = async () => {
    try {
        if (validateForm()) {
            ...
            if(id.value !== null ) 
            handleEditProjectCategory( { ...form.value })
            else
...
// get material
const handleGetProjectCategoryById = async (payload) => {
  loading.value = true;
  try {
    const res = await ProjectServices.getProjectCategory(payload);
...
const handleEditProjectCategory = async (payload) => {
  ...
  try {
    const res = await ProjectServices.editProjectCategory({...form.value});
    if (res.status === 200) {
      showToast(res.data.message, 'success');
      router.push('/Project-category');
    } else if (res.status === 400) {
...
onMounted(()=>{
    if(props.id !== undefined && props.id !== null && props.id !== ' ' ) {
        ...
        handleGetProjectCategoryById({id:props.id,domain_id:store.getters.getDomain.id});
