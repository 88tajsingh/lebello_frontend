...
const { handleGlobalUpdate  } = useGlobalUpdate(ProjectServices.globalProjectsCategoryUpdate,'master_project_category_id');
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (status === 200 && data.success) {
            if (hasCheckedFields) {
                handleGlobalUpdate(form, checkedFields, '/Project-category');
            }
            else {
