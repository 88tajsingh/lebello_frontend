<template>
  ...
  <div class="flex content-between justify-between px-1 mb-2">
    ...
    <div class="flex">
      ...
      <Button v-if="permissions.write"
        @click="() => { router.push({ name: 'Post-form' }); store.dispatch('clearEditData'); }" class="px-2 py-2">Add
        Post</Button>
    </div>
  </div>
  <div class="bg-white rounded-[20px]">
    <vue3-datatable class="next-prev-pagination" ref="datatable" skin="bh-table-striped bh-table-hover "
      :hasCheckbox="true" :cloneHeaderInFooter="true" :stickyHeader="false" :rows="rows" :columns="PostCols"
      ...
      <template v-if="permissions.write" #actions="data">
        <div class="flex gap-3">
          <div @click="() => { router.push({ name: 'Post-form' }); store.dispatch('setEdit', data.value); }"
            id="edit svg">
  ...
  <DeleteModal v-model:isOpen="bulkPopup" :modalTitle="'Delete Post '" @delete="handleBulkActions()">
    Do you want to delete ?
  </DeleteModal>
