  ...
  <div class="flex content-between justify-between px-1 mb-2">
    ...
    <div class="flex">
      ...
      <Button v-if="permissions.write"
        @click="() => { router.push({ name: 'Post-category-form' }); store.dispatch('clearEditData'); }"
        class="px-2 py-2">Add Post Category</Button>
    </div>
  </div>
    ...
    <vue3-datatable class="next-prev-pagination" ref="datatable" skin="bh-table-striped bh-table-hover "
      ...
      <template v-if="permissions.write" #actions="data">
        <div class="flex gap-3">
          <div @click="() => { router.push({ name: 'Post-category-form' }); store.dispatch('setEdit', data.value); }"
            id="edit svg">
...
const handleGetPostCategory = async (payload) => {
  ...
  try {
    const res = await PostServices.getPostCategory(payload);
...
const handleDeletePostCategory = async () => {
  loading.value = true;
  try {
