<template>
  <PageHeader> Product Category Type </PageHeader>
  <div class="flex content-between justify-between px-1 mb-2">
    ...
    <div class="flex">
      ...
      <Button v-if="permissions.write"
        @click="() => { router.push({ name: 'Product-category-type-from' }); store.dispatch('clearEditData'); }"
        class="px-2 py-2">Add Product Category</Button>
    </div>
  </div>
  <div class="bg-white rounded-[20px]">
    <vue3-datatable class="next-prev-pagination" ref="datatable" skin="bh-table-striped bh-table-hover "
      :hasCheckbox="true" :cloneHeaderInFooter="true" :stickyHeader="false" :rows="rows"
      :columns="productCategoryTypeCols" :loading="getLoading" :totalRows="totalRows" :isServerMode="true"
      :pageSize="10" :search="search" @change="changePages">
      <template #name="data">
      ...
      <template v-if="permissions.write" #actions="data">
        <div class="flex gap-3">
          <div @click="() => { router.push({ name: 'Product-category-type-from' }); store.dispatch('setEdit', data.value); }"
            id="edit svg">
  ...
  <DeleteModal v-model:isOpen="deleteModalIsOpen" :modalTitle="'Delete Product Category Type'"
    @delete="handleDeleteProductContractType">
    Do you want to delete?
  </DeleteModal>
  <DeleteModal v-model:isOpen="bulkPopup" :modalTitle="'Delete Product Category Type'" @delete="handleBulkActions()">
    Do you want to delete ?
  </DeleteModal>
...
import { productCategoryTypeCols } from '@/json/data';
...
const handleDeleteProductContractType = async () => {
  loading.value = true;
  try {
    const res = await ProductServices.deleteProductCategoryType({ id: editData.value });
    if (res.status === 200 && res.data.success) {
...
const handleBulkActions = async () => {
  ...
  if (!ids.length) return showToast('Please select atleast one product category type to delete', 'error');
