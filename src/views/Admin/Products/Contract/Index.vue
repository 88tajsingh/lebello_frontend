<template>
    <PageHeader> Product Contract </PageHeader>
    ...
    <div class="bg-white rounded-[20px]">
      ...
      :rows="rows" :columns="productContractCols" :loading="getLoading" :totalRows="totalRows" :isServerMode="true" :pageSize="10" :search="search" @change="changePages">
        <template #name="data">
        ...
        <template #actions="data">
          <div class="flex gap-3">
            <div @click="() =>{router.push({name:'Product-contract-from'}); store.dispatch('setEdit', data.value);}" id="edit svg">
    ...
    <DeleteModal v-model:isOpen="deleteModalIsOpen" :modalTitle="'Delete Product Contract'" @delete="handleDeleteContractLocation">
      Do you want to delete?
    </DeleteModal>
    <DeleteModal v-model:isOpen="bulkPopup" :modalTitle="'Delete Product Contract'" @delete="handleBulkActions()">
    Do you want to delete ?
  </DeleteModal>
...
const handleDeleteContractLocation = async () => {
  loading.value = true;
  try {
    const res = await ProductServices.deleteProductContract({ id: editData.value });
    if (res.status === 200 && res.data.success) {
...
const handleBulkActions = async () => {
  ...
  if (bulkActionSelected.value === 'Delete') {
    loading.value = true;
    try {
      const res = await ProductServices.BulkDeleteProductContract({ id: ids });
      if (res.status === 200 && res.data.success) {
        showToast(res.data.message, 'success');
        await handleGetProductContract();
      }
      else if (res.status === 400){
        showToast(res.data.message, 'error');
      }
