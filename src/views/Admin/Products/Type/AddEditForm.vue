<template>
    <DefaultCard  :cardTitle="form.id ? `Edit Product Type ` : `Add Product Type`">
        <DomainComponent  @customChange="(id)=>form.domain_id = id"
          :deleteService="ProductServices.deleteProductType" masterKey="master_product_type_id"
          :masterDeleteService="ProductServices.mastetrDeleteProductType" routeTo="product-type"
          />
          <template v-if="form.id" v-slot:header>
            <MasterSlugForm
        :form="form"
        @update-slug="()=>fetchProductTypeData()"
        :SlugUpdateservices = 'ProductServices.masterSlugUpdateProductType'
        masteridKeyName='master_product_type_id'
      />
    </template>
        ...
        <div class="p-6.5 grid grid-cols-2 gap-6">
            ...
            <div class="flex flex-col ">
                <TextInput type="text" class="block mr-2 h-[40px] w-full" 
                    placeholder="" v-model="form.slug" label="Slug" disabled="true" />
                <p class="text-sm text-[#646970] text-[11.5px]">
            ...
            <div class="flex flex-col ">
                ...
                <Select :options="getProductTypeList" :defaultZero='true' showfield="name" class="w-full" valueField="id" label="Select "
