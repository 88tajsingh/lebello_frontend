<template>
  ...
  <div class="flex content-between justify-between px-1 mb-2">
    ...
    <div class="flex">
      ...
      <Button v-if="permissions.write"
        @click="() => { router.push({ name: 'Product-type-from' }); store.dispatch('clearEditData'); }"
        class="px-2 py-2">Add Product Type</Button>
    </div>
  </div>
  <div class="bg-white rounded-[20px]">
    <vue3-datatable class="next-prev-pagination" ref="datatable" skin="bh-table-striped bh-table-hover "
      :hasCheckbox="true" :cloneHeaderInFooter="true" :stickyHeader="false" :rows="rows" :columns="productTypeCols"
      ...
      <template v-if="permissions.write" #actions="data">
        <div class="flex gap-3">
          <div @click="() => { router.push({ name: 'Product-type-from' }); store.dispatch('setEdit', data.value); }"
            id="edit svg">
  ...
  <DeleteModal v-model:isOpen="deleteModalIsOpen" :modalTitle="'Delete Product Type'" @delete="handleDeleteProductType">
    Do you want to delete?
  </DeleteModal>
  <DeleteModal v-model:isOpen="bulkPopup" :modalTitle="'Delete Product Type'" @delete="handleBulkActions()">
    Do you want to delete ?
  </DeleteModal>
...
const changePages = (page) => {
  ...
  handleGetProductType(payload);
}
// api calls
const handleGetProductType = async (payload) => {
...
const handleDeleteProductType = async () => {
  loading.value = true;
  try {
    const res = await ProductServices.deleteProductType({ id: editData.value });
    if (res.status === 200 && res.data.success) {
...
const handleBulkActions = async () => {
  ...
  if (!ids.length) return showToast('Please select atleast one product type to delete', 'error');
...
watch(
  ...
  () => {
    ...
    handleGetProductType({ limit: 10, page: 1, domain_id: domain_id.value });
  }
);
</script>