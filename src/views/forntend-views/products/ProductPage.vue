          ...
          <ul class="font-graphikLight text-[13px] my-1 text-textColorBlack overflow-auto max-h-52 ">
            <PerfectScrollbar class="max-h-52">
              <li class="mt-1" v-for="(listItem, index) in products.product_series_data" :key="index">
                <a @click="handelProductSeriesNavigation(listItem)" class="hover:text-orange">{{ listItem?.name }}</a>
        ...
        <div class="flex mt-2">
          ...
          <div class="group relative cursor-pointer">
            ...
            <div @mouseenter="dropdownHoverColor = true" @mouseleave="dropdownHoverColor = false"
              ...
              <a v-for="link in products.product_types" :key="link.name"  class="block border-b text-[12px] border-[#000000AA] py-1 px-2 font-graphikLight text-white hover:text-green">
               <span @click="handleProductType(link)" >{{ link.name }}</span> 
              </a>
            </div>
          </div>
        </div>
