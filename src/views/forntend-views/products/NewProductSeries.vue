<template>
    <div class="relative">
        ...
        <img class="absolute top-8 right-0 mx-auto" src="/src/assets/logo/logohori.png" />
        <!-- side menu  -->
        <div class="relative">
            <div class="absolute top-44 right-0" ref="closeMenu" :class="{ 'w-0': isOpenSidebarSlider }">
                <div class="">
                    <SideMenu menuClass="p-2 bg-[#9ce163]" svgSize="30px" svgColor="white"
                        openClass="w-[230px] absolute z-50 top-0 right-0 "
                        ...
                        <div class="z-50">
                                ...
                                <div class="h-auto">
                                    <h3 class="text-[14px] font-medium">
                                        <a href="https://www.lebello.com/listItem/"
                                            class="uppercase text-graphikMedium text-[14px] text-textColorBlack">Collection
                                            2024</a>
                                    </h3>
                                </div>
                            </div>
                            <div class="px-4">
                                ...
                                <div class="h-auto mb-2 border-y border-[#33333357] py-2 border-b border-gray-400">
                                    <h3 class="text-[14px]">
                                        <a href="https://www.lebello.com/listItem/"
                                            class="uppercase font-graphikMedium text-[14px] text-textColorBlack">Collection
                                            2024</a>
                                    </h3>
                                </div>
                                <ul
                                    class="font-graphikLight text-[13px] my-1 text-textColorBlack overflow-auto max-h-52 ">
                                    <PerfectScrollbar class="max-h-52">
                                        <li class="mt-1" v-for="(listItem, index) in products.product_series_data"
                                            :key="index">
                                            <a @click="handelProductSeriesNavigation(listItem)"
                                                class="hover:text-orange">{{ listItem?.name
                                                }}</a>
                                ...
                                <div class="border-t border-[#33333357] my-3">
                                    <ul class="flex mt-2 ">
                                        ...
                                        <!-- Houzz -->
                                        <li>
                                            <Houzz bgColor="#333333" bgSize="33px" svgSize="15px" svgColor="#FFFFFF"
                                                hoverBgColor="#ce8d39" hoverSvgColor="#ffffff"
                                                href="https://www.houzz.com" title="Houzz Share" />
                                        </li>
                                        <!-- Pinterest -->
                                        <li>
        ...
        <div class="py-5 mx-7 md:mx-20">
            <div class="py-10 ">
                ...
                <p class="font-graphikLight font-light text-[17px] text-textColorBlack">
                    Lebello is an exclusive outdoor furniture manufacturer of innovative outdoor designs for home
                    residential and
                    commercial hospitality projects. The lebello range offers, sofas, tables, lounger and various
                    timeless outdoor
                    furnishings. </p>
                <div class="flex mt-2">
                    ...
                    <div class="group relative cursor-pointer">
                        <div
                            class="flex items-center justify-between hover:bg-[#f8f8f8cc] text-[13px] text-black border-black px-2 bg-gray-100 border rounded-3xl">
                            <a class="menu-hover font-graphik uppercase mx-2 py-1">Product Type</a>
                            <Arrow size="12px" strokeWidth="22.5" initialRotation="left" direction="left"
                                fillColor="black" />
                        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-15 mb-5 ">
                <div v-for="(product, index) in products?.product_series_data || []" :key="product.id"
                    class="prod_content overflow-hidden pb-6 bg-[#FFFBF7] max-h-fit  hover:bg-[#efefef]"
                    ...
                    <div class="relative overflow-hidden h-fit">
                        <a :href="product?.link">
                            <img @click="handelProductSeriesNavigation(product)"
                                class="opacity-60 transition-transform object-contain duration-700 ease-in-out transform hover:scale-110 hover:opacity-100 max-h-[250px] w-full "
                                ref="element" :data-aos="animationType"
                                :src="$filePath(product?.featured_image_data?.file_url)" :alt="product?.name" />
                        </a>
                    ...
                    <div class="max-h-[186px] overflow-y-auto wrapper">
                        <div class="group">
                            <div class="px-7 py-3 hover:bg-[#cc9933] group-hover:bg-[#cc9933]">
                                <h1
                                    class="text-[20px] text-black font-bold font-graphik hover:text-white group-hover:text-white">
                                    {{ product?.name }}
                                </h1>
                            </div>
                        </div>
                        <div class="text-[18px] px-7 font-light text-black">
                            <div>4L Lounger</div>
                            <div class="text-[#cc9933]">4L Pixie</div>
                            <div>4L Stools</div>
                            <div>4L X Series</div>
                            <div>4L X Series</div>
                            <div>4L X Series</div>
                    ...
                    <!-- <p v-for="(prod, index) in product.products" @click="handelProductDetailNavigation(prod)"
                        class="pl-5 pt-1 pb-3 cursor-pointer text-[#3d3d3d] font-graphikLight text-[20px]">{{
                        prod?.title }}
                    </p> -->
        ...
        <div class=" flex justify-center items-center cursor-pointer">
            <span class="w-fit text-[12px] leading-[23px] border-black border px-8 py-2 rounded-3xl">
                SHOW MORE
            </span>
        </div>
...
/* Wrapper for the custom scrollbar */
.wrapper {
  position: relative;
  width: 100%;
  height: 100%;
...
.wrapper::-webkit-scrollbar-thumb {
  background-color: #E5DED6; /* Scrollbar thumb color */
  border-radius: 5px;
}
...
.wrapper {
  ...
  scrollbar-color: #E5DED6 transparent; /* Thumb and track colors */
}
</style>