<template>
    <div class="relative ">
      ...
      <div class="py-5 mx-7 md:mx-20">
        <div class="py-10 ">
          <div class="uppercase font-graphik mb-3 text-[24px] text-[#3d3d3d]">Search: {{slug}} </div>
          <p class="font-graphikLight text-[14px] text-[#4c4c4c]">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div v-for="(product, index) in searchData" :key="product.id"
  ...
  import { Facebook, Menu, Instagram, Houzz, Pinterest, Arrow,   } from "@/components/frontend-components/Svg/Icons";
  import ArrowSvg from "@/components/frontend-components/Svg/Arrow-Svg.vue";
  import { onClickOutside } from '@vueuse/core'
  import { useRouter } from "vue-router";
  import { globalSearch } from "@/helper/frontendHelpers";
  ...
  const slug = ref(router.currentRoute.value.query.search);
  ...
  const handleSearch = async () => {
    loading.value= true
    const { status, data } = await globalSearch(slug.value);
